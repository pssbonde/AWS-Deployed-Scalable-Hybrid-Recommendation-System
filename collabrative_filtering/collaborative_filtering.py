...
# set paths
# output paths
track_ids_save_path = "track_ids.npy"
filtered_data_save_path = "collab_filtered_data.csv"
interaction_matrix_save_path = "interaction_matrix.npz"

# input paths
songs_data_path = "cleaned_data.csv"
user_listening_history_data_path = "User Listening History.csv"

from typing import Any

def filter_songs_data(songs_data: pd.DataFrame, track_ids: list, save_df_path: str) -> Any:
    """
...
def create_interaction_matrix(history_data: dd.DataFrame, track_ids_save_path, save_matrix_path, songs_data: pd.DataFrame = None, song_name: str = None, artist_name: str = None, k: int = None) -> csr_matrix:
    # make a copy of data
    ...
    # get the s (variable) row_indices: Any
    n_tracks = row_indices.nunique()
    n_users = col_indices.nunique()

    # create the sparse matrix
    interaction_matrix_sparse = csr_matrix((values, (row_indices, col_indices)), shape=(n_tracks, n_users))

    # save the sparse matrix
    save_sparse_matrix(interaction_matrix_sparse, save_matrix_path)

    # If songs_data, song_name, artist_name, and k are provided, do recommendations
    if songs_data is not None and song_name is not None and artist_name is not None and k is not None:
        ...
        input_array = interaction_matrix_sparse[ind]

        # get similarity scores
        similarity_scores = cosine_similarity(input_array, interaction_matrix_sparse)
...
def main():
    ...
    # create the interaction matrix
    create_interaction_matrix(user_data, track_ids_save_path, interaction_matrix_save_path, songs_data=songs_data)
