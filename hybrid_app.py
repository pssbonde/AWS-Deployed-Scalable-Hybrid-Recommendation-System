...
# These files (content_based_filtering.py, etc.) must be in the same directory.
...
# Assuming a similar function exists for content-based filtering.
...
#     """Loads the primary songs metadata."""
#     df = pd.read_csv(path)
#     # Pre-process names for easier matching
...
#             ['Hybrid Recommender System', 'Collaborative Filtering', 'Content-Based Filtering'], # Add 'Content-Based Filtering' if you have the function
#             index=0,
#             key="filtering_type"
#         )
...
#             data_to_check = filtered_songs_data if filtering_type != 'Content-Based Filtering' else songs_data
            
#             input_song_query = (data_to_check["name"] == song_name) & (data_to_check["artist"] == artist_name)
            
#             if not input_song_query.any():
#                 st.error(f"Sorry, '{song_name_input}' by '{artist_name_input}' was not found in our database. Please check the spelling or try another song.")
#             else:
...
#                         weight_content_based=0.5, # Example weight
#                         weight_collaborative=0.5, # Example weight
#                         songs_data=filtered_songs_data,
#                         transformed_matrix=transformed_hybrid_data, # Corrected parameter
#                         interaction_matrix=interaction_matrix,     # Corrected parameter
#                         track_ids=track_ids                        # Added missing parameter
#                     )
...
#                         st.write("---")
#                 else:
#                     st.warning("Could not generate recommendations for this song.")

# except FileNotFoundError as e:
#     st.error(f"Fatal Error: A required data file was not found.")
#     st.error(f"Please make sure '{e.filename}' is in the correct directory relative to the script.")
...
# from hybrid_recommendations import HybridRecommenderSystem
# from collaborative_filtering import get_collaborative_recommendations
# # Make sure you have a 'content_based_filtering.py' file with this function
# from content_based_filtering import get_content_based_recommendations
...
def load_songs_data(path):
    ...
    df = pd.read_csv(path)
    # Standardize names for easier matching
    df['name'] = df['name'].str.lower().str.strip()
    df['artist'] = df['artist'].str.lower().str.strip()
    return df
...
try:
    # --- 1. Define File Paths ---
    # This makes it easy to manage your data files.
    cleaned_data_path = "cleaned_data.csv"
    filtered_data_path = "collab_filtered_data.csv"
    ...
    # Using the cached functions for speed.
    songs_data = load_songs_data(cleaned_data_path)
    filtered_songs_data = load_songs_data(filtered_data_path)
